...
const DigitalCard: React.FC<DigitalCardProps> = ({ member, isDependent = false, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`relative aspect-[1.586/1] rounded-[5cqi] p-[6cqi] text-white shadow-[0_8cqi_16cqi_-3cqi_rgba(0,0,0,0.5)] transition-all duration-700 overflow-hidden z-10 border border-white/10 ${isActive
        ...
        <div className="absolute bottom-0 right-0 w-[50cqi] h-[50cqi] bg-orange-600/[0.03] blur-[15cqi] rounded-full"></div>
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between shrink-0 mb-auto">
            <div className="flex items-center space-x-[2.5cqi]">
              ...
              <div className="flex flex-col">
                <h2 className="text-[3.8cqi] font-black tracking-tight leading-none italic uppercase opacity-95">AMABE</h2>
                <p className="text-[2.4cqi] font-black tracking-[0.25em] text-[#FF5C12] uppercase mt-[0.5cqi] italic">Elite</p>
              </div>
            </div>
            ...
            <div className={`px-[3cqi] py-[1.2cqi] rounded-full flex items-center space-x-[1.8cqi] border transition-all duration-500 ${isActive ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-slate-500/10 border-slate-500/30 text-slate-400'}`}>
              ...
              <span className="text-[2.4cqi] font-black uppercase tracking-widest leading-none">{isActive ? 'Ativo' : 'Inativo'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-[3.5cqi] my-auto">
            <div className="relative shrink-0">
              <div className="w-[20cqi] h-[20cqi] rounded-[4cqi] border-[0.6cqi] border-white/20 overflow-hidden shadow-2xl relative">
                <img
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <h3 className="text-[6cqi] font-black leading-tight truncate tracking-tight uppercase italic text-white drop-shadow-2xl">{member.name}</h3>
              <div className="flex flex-col space-y-[0.8cqi] mt-[1cqi]">
                <div className="flex items-center space-x-[1.8cqi]">
                  ...
                  <p className="text-[2.8cqi] font-black tracking-widest text-white/80 uppercase truncate leading-none">
                    {isDependent ? `${member.relationship} • ` : ''}
                    {member.memberId || 'N/A'}
                  </p>
                </div>
                {(member.cpf || member.rg) && (
                  <p className="text-[2.2cqi] font-bold tracking-[0.12em] text-white/50 uppercase leading-none pl-[4cqi] truncate">
          ...
          {/* Footer */}
          <div className="flex items-end justify-between shrink-0 mt-auto">
            <div className="flex items-end gap-[4.5cqi] min-w-0 flex-1">
              {/* Membro Desde */}
              <div className="flex flex-col gap-[0.5cqi] shrink-0">
                <p className="text-[2.2cqi] uppercase text-[#FF5C12] font-black tracking-[0.2em] italic leading-none opacity-90">Membro desde</p>
                <p className="text-[3.5cqi] font-black italic text-white leading-none">2024</p>
              </div>

              {/* Nascimento */}
              <div className="flex flex-col gap-[0.5cqi] min-w-0">
                <p className="text-[2.2cqi] uppercase text-[#FF5C12] font-black tracking-[0.2em] italic leading-none opacity-90">Nascimento</p>
                <div className="flex items-center space-x-[1.5cqi] bg-white/5 px-[2.5cqi] py-[1.5cqi] rounded-[2cqi] border border-white/10 backdrop-blur-sm shadow-inner overflow-hidden">
                  ...
                  <span className="text-[2.8cqi] font-black italic text-white tracking-tight leading-none truncate">{member.birthDate || '--/--/----'}</span>
                </div>
              </div>
            </div>

            <div className="shrink-0 ml-[2cqi]">
              <div className="bg-white p-[0.6cqi] rounded-[1.8cqi] shadow-2xl">
                <img
